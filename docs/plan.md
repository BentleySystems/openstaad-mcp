...
- **Execution limits.** 30s wall-clock timeout, 128 MiB WASM memory, 256 KiB stdout/stderr, 256 KiB max code size.
...
## Future simplifications (post file-I/O)

### Replace `console` with a bounded `__warnings` array

`console.log` and `console.error` are currently pre-injected globals that capture output into `stdout_buf`/`stderr_buf` and return it alongside the result. The original justification was crash diagnostics (partial progress visible when code throws mid-execution) and separating commentary from data.

Problems with the current approach:
- Agents produce chatty debug output that wastes tokens in MCP responses.
- Every byte of stdout/stderr inflates context cost on the host side.
- There is no streaming. Output arrives all at once with the result. It is not progress reporting.
- Structured return values (`{data: [...], warnings: [...]}`) handle the same use cases more efficiently.

**Proposed replacement:** Remove `console` as a pre-injected global. Add a `__warnings` array (frozen reference, mutable contents, capped at 50 entries). Code pushes diagnostic strings to `__warnings`. Server returns them in the response under a `warnings` field. Same diagnostic value, bounded token cost, no open-ended logging channel.

The crash diagnostic use case is handled by try/catch patterns in user code that return partial results on error. The agent is generating the code anyway, so it can always wrap the outer loop in try/catch.

**Security impact:** Reduces sandbox surface by one pre-injected callable. `__warnings` is an array (data), not a function (capability). Strictly simpler.

**When:** After file I/O ships and is stable. Not blocking anything. Console works fine for now.

## ADR: Anthropic policy as primary security governance

We need a formal Architecture Decision Record documenting that openstaad-mcp follows the Anthropic Software Directory Policy as its primary security governance, with Bentley's internal security architecture (`plan-research-support/security-architecture.md`) as secondary.

**Why this needs documenting:** Bentley's security architecture is designed for cloud-hosted, multi-tenant MCP servers calling Bentley backend APIs via IMS tokens. Controls 1 (User-Context AuthZ), 2 (Discovery Service), 6 (AI/LLM Gateway), 7 (Input Parameterization), 8 (Server Proxying), and §9 (Multi-Tenancy) don't apply to a single-user desktop COM bridge with no backend API layer. The Anthropic policy is the actual gating requirement for Extension Store acceptance.

**What the ADR should cover:**
- Each Bentley control mapped to our deployment (applies / does not apply / satisfied differently)
- §10 (Code Mode isolation) applies and is fully satisfied
- Anthropic policy sections (§1B, §1D, §2B, §2D, §5A, §5B, §5E) are directly testable and are what reviewers will check
- If we ever move to cloud/multi-tenant, this ADR gets superseded and the full Bentley architecture applies

**Format:** Follow the Bentley ADR template (Title, Date, Stakeholders, Status, Context, Decision, Consequences). File goes in `/docs/`.
